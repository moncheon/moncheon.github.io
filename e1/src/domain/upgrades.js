...
export const UPGRADE_CATALOG = Object.freeze([
  upgrade('chain-arc', THEME.CHAIN, 'CIRCUIT', '감전 도약', '명중 시 190px 안의 가장 가까운 미타격 적에게 기본 13 감전 피해가 1회 도약한다.', [{ type: 'stat.add', stat: 'chainJumps', value: 1 }]),
  ...
  upgrade('chain-return', THEME.CHAIN, 'RETURN', '귀환 탄환', '탄환이 수명의 절반에서 되돌아오며 다시 적중한다.', [{ type: 'flag.enable', flag: 'returning' }], { quality: [Q.DECISION, Q.CEILING, Q.VISUAL] }),
  upgrade('chain-pierce', THEME.CHAIN, 'RETURN', '관통 회수', '탄환 관통 +2. 왕복 경로의 표적 수가 늘어난다.', [{ type: 'stat.add', stat: 'pierce', value: 2 }]),
  ...
  upgrade('chain-reactor', THEME.CHAIN, 'REACTOR', '시체 반응로', '처치한 적이 작은 폭발을 일으킨다.', [{ type: 'stat.add', stat: 'explosionRadius', value: 52 }], { quality: [Q.IMPACT, Q.DECISION, Q.VISUAL] }),
  ...
  upgrade('return-mastery', THEME.CHAIN, 'RETURN', '귀환 숙련', '투사체 속도 +12%, 관통 +1.', [{ type: 'stat.multiply', stat: 'projectileSpeed', value: 1.12 }, { type: 'stat.add', stat: 'pierce', value: 1 }], { repeatable: true, rarity: 'MASTERY' }),
  ...
  upgrade('bloom-leech', THEME.BLOOM, 'LEECH', '흡혈 수확', '수확 회복량이 두 배가 된다.', [{ type: 'stat.multiply', stat: 'harvestHeal', value: 2 }]),
  upgrade('bloom-armor', THEME.BLOOM, 'LEECH', '생체 갑각', '최대 체력 +20%.', [{ type: 'stat.multiply', stat: 'maxHp', value: 1.2 }]),
  upgrade('bloom-dash', THEME.BLOOM, 'LEECH', '공생 도약', '대시 재사용 시간이 25% 감소한다.', [{ type: 'stat.multiply', stat: 'dashCooldown', value: .75 }]),
  upgrade('bloom-overdose', THEME.BLOOM, 'OVERDOSE', '과다복용', '수확 피해 +75%.', [{ type: 'stat.multiply', stat: 'harvestDamage', value: 1.75 }]),
  upgrade('bloom-compression', THEME.BLOOM, 'OVERDOSE', '스택 압축', '스택 6 이상 수확은 작은 폭발을 일으킨다.', [{ type: 'flag.enable', flag: 'harvestBurst' }], { quality: [Q.IMPACT, Q.DECISION, Q.VISUAL] }),
  ...
  upgrade('hijack-aim', THEME.HIJACK, 'RETURN', '빌린 조준', '반사체의 보스 추적력이 크게 증가한다.', [{ type: 'flag.enable', flag: 'borrowedAim' }], { quality: [Q.IMPACT, Q.DECISION, Q.VISUAL] }),
  upgrade('hijack-final', THEME.HIJACK, 'RETURN', '최후 진술', '보스 체력 15% 이하에서 반사 피해가 3배가 된다.', [{ type: 'flag.enable', flag: 'finalWord' }], { requiresRuleIds: ['hijack.borrowedAim'], rarity: 'EVOLUTION', quality: [Q.SYNERGY, Q.CEILING, Q.VISUAL] }),
  ...
  upgrade('hijack-shield', THEME.HIJACK, 'SHIELD', '방벽 압류', '패링마다 방벽 16을 얻는다.', [{ type: 'stat.add', stat: 'shieldGain', value: 16 }]),
...
export function isUpgradeEligible(definition, themeState, excludedIds = new Set()) {
...
export function applyUpgrade(themeState, definition) {
  validateUpgradeDefinition(definition);
...
export class ChoiceDirector {
  ...
  makeOffer(themeState, excludedIds = new Set()) {
    ...
    const offer = paths.map(path => {
      const candidates = this.catalog.filter(item => item.path === path && isUpgradeEligible(item, themeState, excludedIds));
